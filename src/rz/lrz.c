...
int open_port()
{
	...
	io_mode_fd = open(DEVICE_NAME,O_RDWR | O_NDELAY | O_NOCTTY | O_NONBLOCK);
	if (io_mode_fd == -1)
	{
	...
	{
		fcntl(io_mode_fd, F_SETFL,0);
		printf("Open Successfull\n");
		/* Setting port parameters */
		tcgetattr(io_mode_fd, &options);
		/* control flags */
		cfsetispeed(&options,B115200);    /* 9600 Bds   */
		...
		tcsetattr(io_mode_fd, TCSANOW, &options);
		tcflush(io_mode_fd, TCIFLUSH);
		tcsetattr(io_mode_fd, TCSANOW, &options);
...
#  endif
	...
	readline_setup(io_mode_fd, HOWMANY, MAX_BLOCK*2);
...
et_tu:
	...
	purgeline(io_mode_fd); /* Do read next time ... */
...
get2:
		...
		/* make sure eot really is eot and not just mixmash */
			...
			return WCEOT;
#else
		else if (firstch==EOT && READLINE_PF(1)>0)
			return WCEOT;
#endif
		else if (firstch==CAN) {
			if (Lastrx==CAN) {
...
tryz(void)
{
	...
	for (n=zmodem_requested?15:5; 
		...
		zshhdr(tryzhdrtype, Txhdr,io_mode_fd);
...
again:
		switch (zgethdr(Rxhdr, 0, NULL)) {
			...
			case ZFILE:
				...
				//io_mode(0,3);
				io_mode(io_mode_fd,3);
				if (c == GOTCRCW)
					return ZFILE;
				//zshhdr(ZNAK, Txhdr);
				zshhdr(ZNAK, Txhdr,io_mode_fd);
				goto again;
			case ZSINIT:
				/* this once was:
			...
			case ZCOMMAND:
				...
				if (zrdata(secbuf, MAX_BLOCK,&bytes_in_block) == GOTCRCW) {
					...
					if (!allow_remote_commands) 
					{
						if (Verbose)
							vstringf("%s: %s\n", program_name, 
									_("not executed"));
						//zshhdr(ZCOMPL, Txhdr);
						zshhdr(ZCOMPL, Txhdr,io_mode_fd);
					...
					purgeline(io_mode_fd);	/* dump impatient questions */
					do {
						//zshhdr(ZCOMPL, Txhdr);
						zshhdr(ZCOMPL, Txhdr,io_mode_fd);
					}
				...
				switch (c = zrdata(secbuf+chinseg, MAX_BLOCK,&bytes_in_block))
...
#endif
					{
						...
						case GOTCRCW:
							...
							zshhdr(ZACK | 0x80, Txhdr,io_mode_fd);
							goto nxthdr;
						case GOTCRCQ:
							n = 20;
#ifdef SEGMENTS
...
zmputs(const char *s)
{
	...
	while (s && *s)
	{
		...
		{
			//write(1,s,strlen(s));
			write(io_mode_fd,s,strlen(s));
			return;
		}
		if (p!=s)
		{
			//write(1,s,(size_t) (p-s));
			write(io_mode_fd,s,(size_t) (p-s));
			s=p;
		}
		if (*p=='\336')
			sleep(1);
