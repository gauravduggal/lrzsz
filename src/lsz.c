...
int main()
{
	...
	int i;
	int stdin_files;
	char **patts= (char **)malloc(1024);
	io_mode_fd = open_port();
	//char **patts;
	//*patts="/home/arti/Downloads/zmodem/lrzsz-0.12.20/src/lsz.c > /dev/ttyUSB0 < /dev/ttyUSB0";
	*patts="/home/arti/Downloads/lrzsz-0.12.20.tar.gz";
	//char **patts ;
	int c;
...
#  endif
	...
	if ( protocol!=ZM_XMODEM) {
		...
		if (protocol == ZM_ZMODEM) {
			...
			zshhdr(ZRQINIT, Txhdr, io_mode_fd);
			zrqinits_sent++;
				...
				static int 
					sendzsinit(void)
					{
						...
						for (;;) {
							...
							if (Zctlesc) {
								Txhdr[ZF0] |= TESCCTL; 
								zshhdr(ZSINIT, Txhdr,io_mode_fd);
				...
				static void
					saybibi(void)
					{
						for (;;) {
							...
							zshhdr(ZFIN, Txhdr,io_mode_fd);	/*  to make debugging easier */
