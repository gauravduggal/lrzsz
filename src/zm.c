...
void sendline(int c)
{
	ssize_t nwritten;
	c= ((c)&0377);
	//write(io_mode_fd,(unsigned char)c,sizeof(c));
	nwritten = write(io_mode_fd,&c,1);
...
void 
zshhdr(int type, char *hdr,int fd)
{
	//fd_l =fd;
	printf("io_mode_fd is %d\n",io_mode_fd);
	register int n;
	register unsigned short crc;
	char s[30];
	size_t len;
